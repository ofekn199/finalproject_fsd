...
/**
 * IPostWithLiked — a plain (non-Mongoose-Document) post object enriched with
 * the `isLikedByUser` flag that is computed per-request.
 */
export interface IPostWithLiked {
  _id: Types.ObjectId;
  author: Types.ObjectId | { _id: Types.ObjectId; username: string; profilePicture?: string };
  text: string;
  imageUrl?: string;
  likesCount: number;
...
export async function getFeed(
  ...
  const items: IPostWithLiked[] = posts.map(p => ({
    ...(p.toObject() as unknown as Omit<IPostWithLiked, "isLikedByUser">),
 ...
 */
export async function getPostById(id: string, requesterId?: string): Promise<IPostWithLiked | null> {
  ...
  return {
    ...(post.toObject() as unknown as Omit<IPostWithLiked, "isLikedByUser">),
